...
func TestStorageAccountNamingConventionPolicy(t *testing.T) {
	t.Parallel()

	testCases := []PolicyTestCase{
		{configuration: "asa", errorExpected: true},
		{configuration: "abc", errorExpected: true},
		{configuration: "as", errorExpected: true},
		{configuration: "sa", errorExpected: false},
	}

	beforeAllOptions := &terraform.Options{
		TerraformDir: "./storageaccount-naming/beforeAll",
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("prefix=%s", prefix), func(t *testing.T) {
			...
			terraformDir := test_structure.CopyTerraformFolderToTemp(t, "..", "/test/storageaccount-naming")
