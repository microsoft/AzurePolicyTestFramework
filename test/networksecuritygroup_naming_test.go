...
func TestNetworkSecurityGroupNamingConventionPolicy(t *testing.T) {
	t.Parallel()
	testCases := []PolicyTestCase{
		{configuration: "gsn-", errorExpected: true},
		{configuration: "abc", errorExpected: true},
		{configuration: "abc-", errorExpected: true},
		{configuration: "nsg-", errorExpected: false},
	}

	beforeAllOptions := &terraform.Options{
		TerraformDir: "./networksecuritygroup-naming/beforeAll",
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("prefix=%s", prefix), func(t *testing.T) {
			...
			terraformDir := test_structure.CopyTerraformFolderToTemp(t, "..", "/test/networksecuritygroup-naming")
