...
func TestDisEncryptionSetNamingConventionPolicy(t *testing.T) {
	t.Parallel()
	testCases := []PolicyTestCase{
		{configuration: "sed-", errorExpected: true},
		{configuration: "sed", errorExpected: true},
		{configuration: "se-", errorExpected: true},
		{configuration: "des-", errorExpected: false},
	}

	beforeAllOptions := &terraform.Options{
		TerraformDir: "./diskencryptionset-naming/beforeAll",
	}
	...
	keyVaultKeyID := terraform.Output(t, beforeAllOptions, "keyvault_key_id")

	errorMessagesExpectedParts := []string{
		"Error creating Disk Encryption Set",
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("prefix=%s", prefix), func(t *testing.T) {
			...
			terraformDir := test_structure.CopyTerraformFolderToTemp(t, "..", "/test/diskencryptionset-naming")
