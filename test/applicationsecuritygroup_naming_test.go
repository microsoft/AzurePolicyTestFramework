...
func TestApplicationSecurityGroupNamingConventionPolicy(t *testing.T) {
	t.Parallel()
	testCases := []PolicyTestCase{
		{configuration: "gsa-", errorExpected: true},
		{configuration: "aze", errorExpected: true},
		{configuration: "ns-", errorExpected: true},
		{configuration: "asg-", errorExpected: false},
	}

	beforeAllOptions := &terraform.Options{
		TerraformDir: "./applicationsecuritygroup-naming/beforeAll",
	}
	...
	errorMessagesExpectedParts := []string{
		"Error creating Application Security Group",
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("prefix=%s", prefix), func(t *testing.T) {
			...
			terraformDir := test_structure.CopyTerraformFolderToTemp(t, "..", "/test/applicationsecuritygroup-naming")
